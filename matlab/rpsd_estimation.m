...
L = ceil(sqrt(2)*(patchSz-1)+1); % Number of radial RPSD samples 
...
parfor k = 1:mSquare
    ...
    psdRad = abs(spline(R*bandLimit,rBlock,rhoSamp));
...
rhoMat(sqrt(X.^2 + Y.^2)>bandLimit) = 0;
[rhoSamp,~,idx] = unique(rhoMat);
cleanPsdNodes = abs(spline(R*bandLimit,apprxCleanPsd,rhoSamp));
noisePsdNodes = abs(spline(R*bandLimit,apprxNoisePsd,rhoSamp));
...
sMean = mean(S,2);
sMeanPsdNodes = abs(spline(R*bandLimit,sMean,rhoSamp));
